...
import {EMOJI_PROPERTY_NAME, EMOJI_PROPERTY_UNIFIED} from '../../utils/constants'
import emojiStorage from '../../utils/emojiStorage'
...
import setEmojiName from '../../utils/setEmojiName'
import {
  useActiveCategory,
  useActiveSkinTone,
  useConfig,
...
const useEmojiList = (name, {emojiListRef, searchTerm}) => {
  ...
  const unsetEmojiName = useCallback(() => setEmojiName('', emojiListRef), [emojiListRef])
  ...
  const shouldLoad =
    config.preload || !!(seenGroups[name] || filterResult || typeof globalObject.IntersectionObserver !== 'function')
...
const EmojiList = ({emojiListRef}) => {
  ...
  const searchTerm = filter && filter.length ? filter[filter.length - 1].value : ''
  ...
  useIntersectionObserver(emojiListRef, activeCategoryRef, filterResultRef, renderOne)
  ...
  return (
    <React.Fragment>
      ...
      {!renderOne && groups.slice(1).map(name => <ListRender key={name} name={name} {...props} />)}
    </React.Fragment>
  )
}

const ListRender = React.memo(function ListRender({name, searchTerm, emojiListRef, groupNames}) {
  ...
  return (
    <ul className="emoji-group" data-name={name} data-display-name={groupNames[name]} key={name} style={style}>
      {list}
    </ul>
  )
})
